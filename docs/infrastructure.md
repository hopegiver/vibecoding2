...
전통적인 웹 서비스는 CDN, 로드밸런서, 웹서버, WAS, DB, 스토리지 등 **6개 이상의 서버/서비스를 각각 설정하고 관리**해야 합니다.

**바이브코딩과 맞지 않는 이유:**
- **시스템 엔지니어(DevOps)가 별도로 필요** — AI가 코드를 만들어도 인프라는 사람이 설정
- **초기 구축에 1~2주 소요** — 개발 시작 전에 인프라부터 준비
- **장애 대응이 어려움** — AI가 해결할 수 없는 인프라 영역
- **바이브코딩의 속도를 인프라가 따라가지 못함**
...
Cloudflare가 적합하지 않은 경우도 있습니다:

- **대규모 배치 처리** — Workers CPU 시간 제한 (무료 10ms, 유료 30초)
- **대용량 DB** — D1은 복잡한 쿼리/트랜잭션에 한계 → PlanetScale 전환 가능
- **실시간 영상/음성** — 미디어 서버 필요
- **GPU 연산** — ML/AI 추론

이런 경우에도 **프론트엔드는 Pages, 일반 API는 Workers**에 두고, 특수한 요구사항만 AWS/GCP를 사용하는 하이브리드 구성이 가능합니다.
